...
test('authenticated user can create and delete an article and no console errors occur', async ({
  ...
  await page.getByPlaceholder("What's this article about?").fill('testing article creation');
  await page
  ...
  const topNavProfile = page.locator(`nav a.nav-link[href="${profileHref}"]`).first();
  ...
  const topNavProfile2 = page.locator(`nav a.nav-link[href="${profileHref}"]`).first();
